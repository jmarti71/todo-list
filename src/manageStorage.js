...
function saveSubmitted(element, purpose, title, description, dueDate, priority, completed) {
    ...
    switch (purpose) {
        ...
        case "edit-task":
            ...
            if (!projectSelected(document.querySelector(".selected").id)) {
                console.log("general edit");
                generalList.tasks.splice(findElemIndexByTitle(generalList.tasks, elementOrigTitle), 1, element);
                localStorage.setItem("General", JSON.stringify(generalList));
            }
            // Custom Project Task List
            else if (projectSelected(document.querySelector(".selected").id)) {
                const taskProject = getTaskProject();
                customProjects[findElemIndexByTitle(customProjects, taskProject.title)].tasks.splice(
                    findElemIndexByTitle(customProjects[findElemIndexByTitle(customProjects, taskProject.title)].tasks,
                        elementOrigTitle), 1, element);
        ...
        case "add-task-to-project":
            const projectTask = new Task(title, description, dueDate, priority, completed);
            element.tasks.push(projectTask);
            customProjects.splice(findElemIndexByTitle(customProjects, element.title), 1, element);
            localStorage.setItem("customProjects", JSON.stringify(customProjects));
            console.log("saving new task to project");
            break;
        case "edit-project":
            element.title = title;
            element.description = description;
            element.dueDate = dueDate;
            customProjects.splice(findElemIndexByTitle(customProjects, elementOrigTitle), 1, element);
            localStorage.setItem("customProjects", JSON.stringify(customProjects));
            console.log(element);
            console.log("saving edit project");
            break;
    }
}

function deleteElement(element, type, parent) { //--works but test anyways
    const generalList = getGeneralProject();
    const customProjects = getCustomProjects();
    if (type === "general-task") {
        generalList.tasks.splice(findElemIndexByTitle(generalList.tasks, element.title), 1);
        localStorage.setItem("General", JSON.stringify(generalList));
    }
    else if (type === "project") {
        customProjects.splice(findElemIndexByTitle(customProjects, element.title), 1);
        localStorage.setItem("customProjects", JSON.stringify(customProjects));
    }
    else if (type === "project-task") {
        parent.tasks.splice(findElemIndexByTitle(parent.tasks, element.title), 1);
        customProjects.splice(findElemIndexByTitle(customProjects, parent.title), 1, parent);
        localStorage.setItem("customProjects", JSON.stringify(customProjects));
    }
}
...
function checkLocalStorage() {
    ...
    if (storageLength < 1) {
        const projectsArr = [];
        const generalTasks = new Project("General", "General list of tasks", '2022-09-16', [], false);
...
const getCustomProjects = () => {
    assessProjectCompletion(JSON.parse(window.localStorage.getItem("customProjects")));
    ...
    if (projects.length >= 1) {
        for (let i = 0; i < projects.length; i++) {
            ...
            if (projects[i].tasks.length >= 1) {
                completedTasks == projects[i].tasks.length ? projects[i].completed = true : projects[i].completed = false;
                projects.splice(findElemIndexByTitle(projects, projects[i].title), 1, projects[i]);
...
const sampleTask = () => {
    const task = new Task(
        ...
        "Low",
        false);
    const sampleTasks = new Project("General", "General list of tasks", '2022-09-16', [task], false);
    return sampleTasks;
}
