...
function renderprojectButtons (element, content) {
    ...
    const btnTitle = ['Add Task', 'Edit Project', 'Delete Project', 'About Project'];
    const btnFunctions = [modalManager().renderForm, modalManager().renderForm, modalManager().renderDelete, modalManager().renderInfo];
    ...
    for(let i = 0; i < projBtns.length; i++) {
        ...
        if(i === 0) {
            button.addEventListener('click', () => { btnFunctions[i](element, "add-task-to-project"); });
        }
        else if(i === 1) {
