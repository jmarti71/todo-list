...
import { deleteElement, getGeneralProject, getCustomProjects} from "./manageStorage";
...
function renderContentHeader(icon, title, content, isProject) {
    ...
    headingIcon.className = "material-symbols-outlined";
    ...
    if (isProject !== true) {
        ...
        addBtn.addEventListener('click', () => { modalManager().renderForm(generalTasks, "new-task"); })
...
function emptyListMessage(listType, appendTo) {
    ...
    if (listType == "task") {
        button.addEventListener('click', () => { modalManager().renderForm(generalTasks, "new-task"); });
    }
    else if (listType == "project") {
        button.addEventListener('click', () => { modalManager().renderForm(customProjects, "new-project"); });
    }
...
function updateCurrentTab (element = "none") {;
    ...
    switch (true) {
        ...
        case projectSelected(selectedTab.id) && !isProject(element):
            ...
            console.log("edit task within project");
            removeContent(pageRef.content);
...
function updateSideBar (editedElementSelected = "none") {
    ...
    if(projectSelected(currentTabID)) {
        editedElementSelected === "none"
           ...
           : document.getElementById(editedElementSelected.title).className = "selected";
    }
}
