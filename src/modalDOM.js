...
const modalManager = () => {
  function generateHiddenModal() {
    ...
    closeBtn.className = "material-symbols-outlined close-button";
  ...
  function renderInfo(element) {
    ...
    _getModalComponent().contentContainer.append(
      title,
      descripion,
      priorityOrTasksDiv,
      due
  ...
  function renderDelete(element) {
    ...
    confirmBtn.className = "delete-button";
    confirmBtn.addEventListener("click", () => {
      processDeletion(element);
      _closeModal(
        _getModalComponent().modal,
        _getModalComponent().contentContainer
      );
  ...
  function renderForm(element, purpose) {
    ...
    submitBtn.className = "submit-modal";
  ...
  function processSubmit(
    ...
    if (validateEntry(title, description, dueDate, priority)) {
      if (purpose === "edit-task" || purpose === "edit-project") {
        if (
          checkForDifference(element, title, description, dueDate, priority)
        ) {
...
function validateName(title, priority, element = "default") {
  if (priority === "none") {
    ...
    if (isNameTaken(getCustomProjects(), title)) {
      ...
      // And name belongs to a different element other than what is being modified
      else {
        displayErrorMessage(`* Project name "${title}" is already taken`);
        return false;
      }
    ...
    if (!projectSelected(document.querySelector(".selected").id)) {
      if (isNameTaken(getGeneralProject().tasks, title)) {
        if (element === "default") {
          displayErrorMessage(
            `* Task name "${title}" is already taken on this list`
          );
          return false;
        } else if (element.title == title) {
          return true;
    ...
    // If editing task within a project
      ...
      if (isNameTaken(getTaskProject().tasks, title)) {
        if (element === "default") {
          displayErrorMessage(
            `* Task name "${title}" is already taken in this project`
          );
          return false;
        } else if (element.title == title) {
          return true;
