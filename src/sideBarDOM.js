...
const sideBar = (sidebar) => {
    ...
    function collapseSideBar() {
        ...
        toHide.forEach(function (item) { item.className = 'side-content hide'; });
        setGridElement("60px");
    ...
    function _expandSideBar() {
        ...
        toShow.forEach(function (item) { item.className = 'side-content show'; });
        setGridElement("275px");
        sidebar.className = "side-bar expanded";
...
function renderSideBarProjects(sidebar) {
    ...
    if (projects.length === 0) {
        ...
        projDiv.addEventListener('click', () => { modalManager().renderForm(projects, "new-project"); });
    }
        ...
        for (let i = 0; i < projects.length; i++) {
            ...
            projDiv.addEventListener('click', () => { removeContent(pageRef.content); loadProject(projects[i], pageRef.content) });
        }
    }
}
...
function renderSideBarProjHead(appendTo) {
    ...
    addProjBtn.className = "material-symbols-outlined";
    addProjBtn.textContent = "add_circle";
    addProjBtn.title = "Add Project";
    addProjBtn.addEventListener('click', () => { modalManager().renderForm(projects, "new-project"); });
